...
# dataRepresentaion creating a RESTful API
...
# curl -X POST "http://127.0.0.1:5000/books/1234"
# internal server error (500) means error in your code
# For windows: (and change the header with -H)   -d is an object
# To create a new author/title etc:
...
def create():
    ...
    if not request.json:
        abort(400) # return about arror

    # other checking for more marks that its properly formatted
    book = {
    "id": nextId,
    ...
    nextId += 1
    books.append(book)
    return jsonify(book) # "in create"
 #   return str(nextId) # should retutn "5"
 # Now do a getAll to test: curl "http://127.0.0.1:5000/books"

# curl -X PUT "http://127.0.0.1:5000/books/1234"
# For example i we want to update /change the 1st book (Harry Potter):
...
# Find the item that i need to update (e.g. book/car etc)
def update(id): 
    foundBooks = list(filter(lambda t: t['id']== id, books)) # = id we passed in (do on book array)
    ...
    # if price was passed up for example: 
    # Error checking/throws message if incorrect input
...
# curl -X DELETE "http://127.0.0.1:5000/books/1234"
